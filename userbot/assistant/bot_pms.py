...
from userbot import Config, xrxnr
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**⌔︙ اهلا وسهلا اخوي  {mention} **\
                        ...
                        \n\n**⌔︙ البوت خاص بسورس :** [TELETHON FOR STARK](https://t.me/XRXNR)"
        buttons = [
            (
                Button.url("تنصيب سورسنا", "https://heroku.com/deploy?template=https://github.com/STARK-LUCIFER/HELL"),
                Button.url(
                    "قناه المساعد",
                    "https://t.me/XRXNR",
                ),
...
@xrxnr.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await xrxnr.tgbot.send_message(
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await xrxnr.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await xrxnr.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await xrxnr.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await xrxnr.tgbot.get_entity(BOTLOG_CHATID)
            await xrxnr.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await xrxnr.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@xrxnr.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await xrxnr.get_entity(user_id)
    except Exception as e:
...
@xrxnr.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@xrxnr.bot_cmd(incoming=True, func=lambda e: e.is_private)
@xrxnr.bot_cmd(edited=True, func=lambda e: e.is_private)
